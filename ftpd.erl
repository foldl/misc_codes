...
%% Updates by Zhengji Li <foldl@outlook.com> Sept 2015:
...
start(Opts) ->
    Tag = make_ref(),
    _Pid = spawn(?MODULE, init, [self(), Tag, Opts]),
    receive
	{Tag,Reply} -> Reply
    end.
...
ctl_loop_init(Ctl, Root, DefaultDataPort) ->
    ...
    rsend(Ctl,220, Name ++ " Erlang Ftp server 1.0 ready. (UTF-8 enabled)"),
...
ctl_parse([L1,L2,L3 | T]) ->
	...
	[$ | Arg] ->
	    ...
	    ctl_parse(list_to_atom([C1,C2,C3]), parse_utf8(Arg));
	[C4] ->
	    ctl_parse(list_to_atom([C1,C2,C3,alpha(C4)]),[]);
	[C4,$  | Arg] ->
	    ctl_parse(list_to_atom([C1,C2,C3,alpha(C4)]), parse_utf8(Arg));
	_ -> error
    end;
ctl_parse(_) -> error.

parse_utf8(Str) -> unicode:characters_to_list(list_to_binary(Str)).
...
help(_, S, St) ->
    rmsend(S, 214,
	   ...
	   "Direct comments to tony@erix.ericsson.se."),
    St.
...
rsend(S, Code) ->
    tcp_send_unicode(S, [rstr(Code)++ ?CRNL]).

%% Send a single line reply with CRNL
rsend(S, Code, Mesg) when is_integer(Code) ->
    tcp_send_unicode(S, [integer_to_list(Code)," ",Mesg, ?CRNL]).

%% send a multi line reply
rmsend(S, Code, Mesg1, Lines, Mesg2) ->
    tcp_send_unicode(S, [integer_to_list(Code),"-",Mesg1, ?CRNL,
...
send_file(Fd, Chunk, Count, S) ->
	...
	{ok,Data} ->
	    case gen_tcp:send(S, Data) of
            ok -> send_file(Fd, Chunk, Count+chunk_size(Data), S);
		Error -> Error
	    end;
	Error -> Error
    end.
...
chunk_size(X) when is_list(X) -> length(X);
chunk_size(X) when is_binary(X) -> size(X).
	...
	{ok, List} ->
	    foreach(
	      fun(E) when Type == nlst ->
		      tcp_send_unicode(S, E ++ ?CRNL);
		 (E) when Type == list ->
		      tcp_send_unicode(S, list_info(Dir1, E) ++ ?CRNL)
	      end,
	      List),
	    rsend(Ctl, 226);
...
tcp_send_unicode(Socket, Str) ->
    gen_tcp:send(Socket, unicode:characters_to_binary(lists:flatten(Str))).
